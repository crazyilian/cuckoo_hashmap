...
struct StrangeInt {
  ...
  friend std::ostream &operator<<(std::ostream &out, const StrangeInt &xx) {
    out << xx.x;
    return out;
  }
};
...
namespace internal_tests { // NOLINT
...
void hash_check() {
  ...
  auto simple_hash = [](uint64_t x) -> size_t {
    return x % 17239;
  };
